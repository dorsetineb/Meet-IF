...
const InputField = ({ id, label, type, value, onChange, min, step }: { id: string, label: string, type: string, value: string | number, onChange: (e: React.ChangeEvent<HTMLInputElement>) => void, min?: string, step?: string }) => (
  <div>
    ...
    <input
      ...
      min={min}
      step={step}
      className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-xs"
    />
  </div>
);
            ...
            <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-end">
                    <div className="sm:col-span-1">
                        <label htmlFor="frequency" className="block text-xs font-medium text-gray-700">Frequência de reuniões por equipe</label>
                        <select id="frequency" name="frequency" value={settings.frequency} onChange={handleChange} className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-xs">
                     ...
                     <div className="sm:col-span-2">
                        <label className="block text-xs font-medium text-gray-700 sm:mb-2">Dias da semana</label>
                        <div className="grid grid-cols-5 gap-2">
                            {ALL_DAYS.map(day => (
                                <button type="button" key={day} onClick={() => handleDayToggle(day)} className={`px-2 py-2 text-xs font-semibold rounded-md transition-colors duration-200 ${ settings.days.includes(day) ? 'bg-primary-600 text-white shadow' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
                                    {day.substring(0, 3)}
                                </button>
                            ))}
                        </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 items-end">
                    ...
                    <div>
                        <label className="block text-xs font-medium text-gray-700 invisible" aria-hidden="true">Almoço</label>
                        <button 
                            type="button" 
                            onClick={onLunchSettingsClick}
                            className={`mt-1 block w-full px-3 py-2 text-xs font-medium rounded-md shadow-sm border transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-500 ${settings.lunchStartTime && settings.lunchEndTime ? 'bg-primary-600 text-white hover:bg-primary-700 border-primary-600' : 'bg-gray-200 text-gray-700 hover:bg-gray-300 border-gray-200 hover:border-gray-300'}`}
                    ...
                    <div>
                        ...
                        <div className="mt-1 flex items-baseline gap-2">
                             <input
                                ...
                                min="5"
                                step="5"
                                className="block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-xs [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                            />
                            <span className="text-xs text-gray-600">minutos</span>
                        </div>
                    </div>
                    <div>
                        <label htmlFor="breakInterval" className="block text-xs font-medium text-gray-700">Intervalo entre reuniões</label>
                         <div className="mt-1 flex items-baseline gap-2">
                            <input
                                type="number"
                                id="breakInterval"
